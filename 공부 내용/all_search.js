...
app.post("/search_result", function(req,res) { // 검색 결과 가져오기
    ...
    var query = "SELECT * FROM contents WHERE REPLACE(title,' ','') LIKE '%" + s + "%' OR REPLACE(director,' ','') LIKE '%" + s + "%' OR content_id IN (SELECT content_pid FROM contents_persons WHERE person_pid IN (SELECT person_pid FROM person WHERE REPLACE(Name,' ','') LIKE '%" + s +"%') );";
